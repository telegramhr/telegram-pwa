...
export default {
  ...
  computed: {
    store() {
      return this.$store.state.games.bee
    },
    otherLetters() {
      return this.store.availableLetters
        .split('')
        .filter((l) => l !== this.store.middleLetter)
    },
    availableLetters() {
      return this.store.availableLetters
    },
  ...
  methods: {
    submitGuess() {
      this.$store.dispatch('games/bee/submitGuess', this.userGuess)
      this.userGuess = ''
    },
    shuffle() {
      this.otherLetters = shuffle(this.otherLetters, Math.random())
    },
    onKeyPress(e) {
      const pressedKey = e.key.toLowerCase()
      if (pressedKey === 'enter')
        return this.submitGuess({ guess: this.userGuess })
      if (['backspace', 'delete'].includes(pressedKey)) {
        this.userGuess = this.userGuess.slice(0, -1)
        return false
      }
      if (
        pressedKey.length === 1 &&
        this.availableLetters.includes(pressedKey)
      ) {
        this.userGuess += pressedKey
        return true
      }
    },
  },
  ...
  <div class="sb-controls" :style="`z-index: ${ZIndex}`">
    ...
    <div class="hive-actions">
      ...
      <button
        ...
        @click="otherLetters = shuffle(otherLetters, Math.random())"
      ></button>
      <button
        class="hive-action hive-action__submit sb-touch-button"
        style="min-width: 5.5em"
        @click="submitGuess({ guess: userGuess })"
      >
        {{ 'Enter' }}
      </button>
    </div>
...
@import '../assets/games/bee/styles/_variables';
...
.hive-action__shuffle {
  background: url(../assets/games/bee/shuffle.svg) center no-repeat;
  background-color: #fff;
